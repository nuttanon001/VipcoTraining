...
@Component({
    ...
    templateUrl: "./group-dialog-data.component.html",
...
export class GroupDialogComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        private serviceCom: GroupDialogDataCommunicateService
    ) { }
    ...
    onLoadData(event: LazyLoadEvent | undefined): void {
        ...
        if (!this.columns) {
            this.columns = [
                ...
                { field: "GroupCode", header: "Code", style: { 'width': '25%' } },
                { field: "GroupDesc", header: "GroupName" },
            ];
        }

        this.service.getAllWithLazyLoad(lazydata)
            .subscribe(restData => {
                this.groups = restData.Data;
    ...
    onSendDataToPartne() {
        if (this.group) {
            this.serviceCom.toParent(this.group);
        }
    }
}